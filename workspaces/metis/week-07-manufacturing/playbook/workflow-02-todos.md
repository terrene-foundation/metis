...
Do NOT propose floors, thresholds, weights, or autonomy slots in any todo.
...
Output directory: workspaces/metis/week-07-manufacturing/todos/active/
Todo naming: phase_N_<slug>.md (e.g. phase_1_frame.md, phase_6_vision.md,
phase_7_rl.md).

Phase coverage: Phases 1–13 (Phase 14 deferred to Week 8). Do NOT create a
todo for Phase 14.

Extra todos required:
- phase_11_postwsh.md and phase_12_postwsh.md for the MOM/WSH injection
  re-runs (these are mandatory, not optional — if they're missing, the D3
  rubric fails)
- phase_{4..8}_predmaint.md for the Sprint 2 time-series replay of phases 4–8

A 14th todo: phase_99_close.md covering /redteam + /codify + /wrapup.

Endpoints: name them; cite from SCAFFOLD_MANIFEST.md (the 33-route inventory
in §"Endpoint inventory"). If an endpoint isn't in the manifest, leave blank
...
- ✓ Extra todos present: `phase_11_postwsh.md`, `phase_12_postwsh.md`, and `phase_{4..8}_predmaint.md` for the Sprint 2 replay
- ✓ A `phase_99_close.md` todo covers `/redteam` + `/codify` + `/wrapup`
- ✓ Each todo names the Trust Plane decision (one sentence), the endpoint(s) cited from `SCAFFOLD_MANIFEST.md`, the skeleton file, and the acceptance criterion
...
- ✗ A todo that proposes a value (e.g. "set safety_critical_defect threshold to 0.85") — ask "please remove the proposed value; I own the decision, you own framing the todo."
...
- ✗ Missing `phase_11_postwsh.md` or `phase_12_postwsh.md` — ask "where is the MOM/WSH re-run plan? The injection is mandatory, not optional."
...
- **Phase ordering** — the canonical sprint sequence (Vision → PredMaint → RL → Agent) that determines which phases run when
...
The explicit assignment of "who sets this value" at plan time. The todo names the decision ("set safety_critical_defect auto-pass threshold subject to WSH 0.40 floor"), not the answer. Decision authority is yours: Claude Code frames the decision space; you write the value into the phase journal. If the todo already contains a proposed value, the Playbook pre-registration is corrupted before you even start the phase.
...
The canonical execution order across four sprints (Vision QC → Predictive Maintenance → RL Optimization → Agent + MLOps), where phases 1–3 are shared across Sprint 1 and Sprint 2, phases 4–8 replay for Sprint 2 (PredMaint), and the MOM/WSH injection forces re-runs of phases 11 and 12 mid-Sprint 3. Getting the ordering wrong at `/todos` means implementing phases out of sequence — the WSH re-run is the highest-risk gap because it looks optional until the rubric says otherwise.
...
building an industrial AI suite for LumenCircuit. I'm currently in the /todos step.

Read `workspaces/metis/week-07-manufacturing/playbook/workflow-02-todos.md` for
what this step does, and read `workspaces/metis/week-07-manufacturing/todos/active/`
...
- [ ] Todos exist for phases 1–13 (Phase 14 absent — deferred to Week 8)
- [ ] `phase_11_postwsh.md`, `phase_12_postwsh.md`, and `phase_{4..8}_predmaint.md` all present
